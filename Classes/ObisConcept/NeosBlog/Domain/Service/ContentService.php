...
class ContentService
{
    ...
    const NODETYPE = 'TYPO3.Neos:Content';
    ...
    public function renderTeaser(NodeInterface $node, $maximumLength = 500)
    {
        ...
        foreach ($node->getNode('content')->getChildNodes('TYPO3.Neos.NodeTypes:Text') as $contentNode) {
    ...
    public function getPostImageResourceObject(NodeInterface $node) {
        ...
        foreach ($childNodes as $childNode) {
            if ($childNode-> getNodeType()->isOfType('TYPO3.Neos.NodeTypes:TextWithImage')) {
                $imageResource[] = $childNode->getProperty('image');

            } elseif ($childNode->getNodeType()->isOfType('TYPO3.Neos.NodeTypes:Image')) {
                $imageResource[] = $childNode->getProperty('image');
    ...
    public function getPostTextTeaser(NodeInterface $node, $maximumLength = 500) {
        ...
        /** @var NodeInterface $childNode */
        foreach ($childNodes as $childNode) {
            if ($childNode-> getNodeType()->isOfType('TYPO3.Neos.NodeTypes:Text')) {
                if($childNode->getProperty('text') !== null ){
                    $teaserText[] = $childNode->getProperty('text');
                }

            } elseif ($childNode->getNodeType()->isOfType('TYPO3.Neos.NodeTypes:TextWithImage')) {
                if($childNode->getProperty('text') !== null ){
                    $teaserText[] = $childNode->getProperty('text');
                }
    ...
    public function renderContent(NodeInterface $node)
    {
        ...
        /** @var NodeInterface $contentNode */
        foreach ($node->getNode('content')->getChildNodes('TYPO3.Neos:Content') as $contentNode) {
